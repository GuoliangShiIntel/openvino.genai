...
void apply_eagle3_attention_mask_transform(std::shared_ptr<ov::Model>& model) {
    ...
    OPENVINO_ASSERT(!sdpa_nodes.empty(), 
                    "Eagle3AttentionMaskTransform: No SDPA nodes found in model");
    ...
    for (const auto& sdpa : sdpa_nodes) {
        ...
        OPENVINO_ASSERT(sdpa->get_input_size() > SDPA_ATTENTION_MASK_INPUT_INDEX,
                       "Eagle3AttentionMaskTransform: SDPA node ", sdpa->get_friendly_name(),
                       " does not have attention_mask input at index ", SDPA_ATTENTION_MASK_INPUT_INDEX);
    ...
    auto add_op = std::make_shared<ov::op::v1::Add>(
        attention_mask_source_node->output(0),
        eagle_tree_mask_param->output(0)
    );
